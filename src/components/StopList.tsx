...
export default ({ trip, vehicle }: { trip?: Trip, vehicle?: Vehicle }) => {
    ...
    return <List>
        {stops?.map(stop => ({
            ...stop,
            delay: Math.round(vehicle?.delay !== undefined ? ((vehicle?.delay || 0) / 60) : (tripStart + stop.time - (lastStop?.time || 0)) * (lastStop && ((nextStop === stop && !serving) || serving === stop) ? percentTravelled(serving || lastStop, stop) : 1) - (next === stop ? 0 : toNextStop) - minutesUntil(stop.arrival)) || 0
