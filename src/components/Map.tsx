...
export default ({ city, children }: {
    ...
    >
        ...
        <div className="leaflet-control-zoom leaflet-bar leaflet-control" style={{ top: 120, right: 10, position: "absolute" }}>
            {cities[city].functions.filter && <a href="/" onClick={e => { navigate(`/${city}/filter`); e.preventDefault(); }} style={{ backgroundColor: (JSON.parse(localStorage.getItem(`${city}.filter.lines`) as string || "[]").length || JSON.parse(localStorage.getItem(`${city}.filter.depots`) as string || "[]").length || JSON.parse(localStorage.getItem(`${city}.filter.models`) as string || "[]").length) ? "#5FDA5D" : "" }}><FilterList sx={{ fontSize: 19, marginTop: 0.75 }} /></a>}
