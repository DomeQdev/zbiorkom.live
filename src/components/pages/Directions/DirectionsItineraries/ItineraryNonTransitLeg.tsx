...
import { NonTransitLeg } from "typings";

export default ({ leg }: { leg: NonTransitLeg }) => {
    if (leg.mode === "RENTAL") {
        return (
            <SvgIcon
                width="64"
    ...
    if (leg.duration > 60000 * 3) {
        const Icon = leg.mode === "WALK" ? DirectionsWalk : DirectionsBike;
        ...
        return (
            ...
            >
                <Icon fontSize="small" />
                <span>{Math.round(leg.duration / 60000)}'</span>
            </Box>
        );
    }
};
